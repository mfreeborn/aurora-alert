...
pub fn locations_field(props: &LocationsFieldProps) -> Html {
    ...
    let onclick = {
        ...
        Callback::from(move |_: MouseEvent| {
            ...
            let mut new_chosen_locations = (*chosen_locations).clone();
    ...
    html! {
        <>
            ...
            <ChosenLocations locations={chosen_locations.clone()} />
        </>
    }
}
...
pub fn chosen_locations(props: &ChosenLocationsProps) -> Html {
    ...
    if let Some(location) = (*location_to_remove).clone() {
        let mut new_locations = (*props.locations).clone();
...
pub fn home() -> Html {
    ...
    let registration_info = UserRegisterPostBody {
        email: (*email_handler).clone(),
        alert_threshold: (*alert_threshold_handler).clone(),
